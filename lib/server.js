    ...
    setupReqRes(req, res, function(err, next) {
      ...
      sessionStore.createSession(req.cookies.get('sid'), function(err, session) {
        ...
        if(err) {
          ...
          req.session = session;
          var route = function() {
            config.loadConfig(options.basepath, server, function(err, resourcesInstances) {
              if (err) throw err;
...
Server.prototype.listen = function(port, host) {
  ...
  config.loadConfig(this.options.basepath, server, function(err, resourcesInstances) {
    if (err) {
      console.error();
